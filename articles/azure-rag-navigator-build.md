...
Azure AI Searchはキーワード検索・ベクトル検索・セマンティックランカーをそれぞれ単独でも使えるが、Microsoft公式ドキュメントでもハイブリッド検索(キーワード+ベクトル)が単独方式より高い関連性を示すケースが多いとされており、各方式の弱点を補い合えるという一般的な期待から、今回は最初からこの3方式を組み合わせる構成を前提として設計した。
...
この一件から得た教訓は2つ。**埋め込み・チャットの両方が同時に失敗する場合は、デプロイ名より先にエンドポイントの値そのものを疑う**こと。そしてサブスクリプションのクォータ制約で「設計時に想定していたモデルがそのままデプロイできるとは限らない」ことを、コードのコメントとして残しておくこと(実際に`index_schema.py`にこの経緯をコメントで残した)。
...
コストとレイテンシの観点で2を選んだ。Cosmos DBは消費リクエストユニット(RU)に基づく課金モデルで、一般に単一パーティション内の点読み・patchはクエリより少ないRUで完結し、低レイテンシになりやすいとされる。ただし今回実際に実測したのは採用した方式(2)のみで、`COUNT(1)`クエリ方式(1)を実装して同条件で比較する検証は行っていない。6.3節のレイテンシ実測値は、あくまで方式2単体の特性であり、方式1との定量比較ではない点に注意されたい。
...
### 7.3 Cosmos DBは「クエリより点操作」を選ぶだけでコスト・レイテンシ特性が変わりうる

6.1節の採番方式の判断がその実例で、`COUNT`クエリの代わりに点読み+patchに寄せる設計にしたことで、2回目以降のターン追加が平均64.5msに収まった。ただし6.1節で触れた通り、`COUNT`クエリ版を実装して同条件で比較したわけではないため、「点操作に寄せたから速くなった」と因果関係を断定はできない。それでも、Cosmos DBのRU課金モデル上「クエリ」と「点操作(read/create/patch)」でコスト特性が異なることを踏まえて設計した結果、実際に64.5msという実用的な値に収まったことは確認できた。
