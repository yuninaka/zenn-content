...
チャットUIの既定チャンク戦略は`heading_aware`にした。見出し単位でまとまりが保たれ、引用元の`section_path`も直感的になる(「在庫管理モジュール設定ガイド > 在庫閾値アラートの設定」のように意味のある単位で表示できる)という理由からだが、この判断が4章のgolden_qa結果と少しねじれることになる(詳細は4.2節)。
